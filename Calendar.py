...
class Calendar:
    ...
    def __place_widgets(self):
        ...
        #时下拉框
        self.CB_hour = ttk.Combobox(bframe, width = 3, values = ['%02d' % hour for hour in range(0,24)], validate = 'key', validatecommand = (Input_judgment_num, '%P'))
        self.CB_hour.current(0)
        self.CB_hour.grid(in_=bframe, column=0, row=0)
        self.CB_hour.bind('<KeyPress>', lambda event:self._update(event, True))
        self.CB_hour.bind("<<ComboboxSelected>>", self._update)
        tk.Label(bframe, text = 'Hour').grid( in_=bframe,column=1, row=0, padx=(0,5))
        #分下拉框
        self.CB_mins = ttk.Combobox(bframe, width = 3, values = ['%02d' % mins for mins in range(0,60)], validate = 'key', validatecommand = (Input_judgment_num, '%P'))
        self.CB_mins.current(0)
        self.CB_mins.grid(in_=bframe, column=2, row=0)
        self.CB_mins.bind('<KeyPress>', lambda event:self._update(event, True))
        self.CB_mins.bind("<<ComboboxSelected>>", self._update)
        tk.Label(bframe, text = 'Minute').grid(column=3, row=0)
        #秒下拉框
        self.CB_seconds = ttk.Combobox(bframe, width = 3, values = ['%02d' % secds for secds in range(0,60)], validate = 'key', validatecommand = (Input_judgment_num, '%P'))
        self.CB_seconds.current(0)
        self.CB_seconds.grid(in_=bframe, column=4, row=0)
        self.CB_seconds.bind('<KeyPress>', lambda event:self._update(event, True))
        self.CB_seconds.bind("<<ComboboxSelected>>", self._update)
        tk.Label(bframe, text = 'Second',justify = 'left').grid(column=5, row=0)
 
        ttk.Button(xframe, text = "Confirm", width = 6, command = lambda: self._exit(True)).grid(row = 1, column = 0, sticky = 'ns', padx = 20)
        ttk.Button(xframe, text = "Cancel", width = 6, command = self._exit).grid(row = 1, column = 4, sticky = 'ne', padx = 20)   
    ...
    def __config_calendar(self):
        ...
        cols = ['Sum','Mon','Tue','Wed','Thur','Fri','Sat']
    ...
    def _update(self, event = None, key = None):
        ...
        self._canvas.place_forget()
        self._date = datetime(year, month, 1,hour,mins,seconds)
    ...
    def selection(self):
        ...
        return str(datetime(year, month, int(self._selection[0]),hour,mins,seconds))
